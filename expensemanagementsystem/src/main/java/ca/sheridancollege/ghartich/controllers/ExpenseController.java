 ...
 * */	
	...
	/// this is a test method to test saving receipts via python. DO NOT REMOVE
    @PostMapping(value = "/testclaim/{employeeId}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<?> testClaimNewExpense(
            @RequestPart("file") MultipartFile file,
            @RequestPart("expense") String expenseJson, // JSON as string
            @PathVariable Long employeeId
    ) throws Exception {
    	try {
	        ...
	        try {
	            RestTemplate restTemplate = new RestTemplate();
	            String url = PYTHON_SERVICES + "save_receipt_data";

	            Map<String, Object> payload = new HashMap<>();
	            payload.put("file_name", tempFile.getName());
	            payload.put("receipt_data", expenseItems);
	            ...
	            e.printStackTrace();
	            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
	                    .body(Map.of("status", 500, "message", "Error saving receipt. Please try again later."));
	        }
	        
	        if(!receiptSaved) {
				return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
						.body(Map.of("status", 502, "message", "Error Saving Result from Python"));
	        }
